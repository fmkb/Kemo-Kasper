...
// if no 'order by' clause found, apply default sorting if specified
	...
	if(!$Print_x) 
		...
		$this->HTML .= "<div class=\"TableBody\" style=\"text-align:center;\"><nobr>";
		// display Print icon
...
// End of templates
	...
	if(!$this->HideTableView || in_array(dirname(__FILE__)."\\separateDVTV.php", get_included_files()) || in_array(dirname(__FILE__)."/separateDVTV.php", get_included_files())){
		// filters ...
		$FiltersCode = "";
		for($i = 1; $i <= (20 * $FiltersPerGroup); $i++){ // Number of filters allowed
			if($i%$FiltersPerGroup == 1 && $i != 1 && $FilterAnd[$i]!=""){
			...
			while(($row = mysql_fetch_row($result)) && ($i < ($FirstRecord + $this->RecordsPerPage)))
			{
				...
				if($rowTemplate!=''){
					...
					if($this->AllowSelection == 1 && $SelectedID != $row[$FieldCount]){
						$rowTemp = str_replace('<%%SELECT%%>',"<a href=\"javascript:document.myform.SelectedID.value='" . addslashes($row[$FieldCount]) . "'; document.myform.submit();\" class=$class style=\"padding:0px;\">",$rowTemp);
	...
	// end of data
		...
		if($this->AllowSelection)
		{
			$this->HTML .= "\n\t<tr><td colspan=" . ($FieldCount + 2) . "> " . form($SelectedID, $this->AllowUpdate, (($this->HideTableView && $SelectedID) ? 0 : $this->AllowInsert), $this->AllowDelete) . " </td></tr>";
		}
		$this->HTML .= "</table>\n";
