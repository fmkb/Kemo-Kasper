...
$xt->assign("langselector_attrs","name=lang onchange=\"javascript: window.location='Dialog_list.php?language='+this.options[this.selectedIndex].value\"");
...
//	validation stuff
	...
	$includes.="<script type=\"text/javascript\" src=\"include/inlineedit.js\"></script>\r\n";
			...
			$addValidateFields[] = "d_from_p_id";
						$validatetype="";
					$addValidateTypes[] = $validatetype;
			$addValidateFields[] = "d_to_p_id";
			$addValidateTypes[] = "";
		$addValidateFields[] = "d_message";
			$addValidateTypes[] = "";
		$addValidateFields[] = "d_datetime";
										$validatetype="IsNumeric";
					$addValidateTypes[] = $validatetype;
			$addValidateFields[] = "d_msg_type";
...
"\nvar bLoading=false;\r\n";
	...
	$includes.="var INLINE_EDIT_TABLE='Dialog_edit.php';\r\n";
	$includes.="var INLINE_ADD_TABLE='Dialog_add.php';\r\n";
	$includes.="var INLINE_VIEW_TABLE='Dialog_view.php';\r\n";
	$includes.="var SUGGEST_TABLE='Dialog_searchsuggest.php';\r\n";
	$includes.="var MASTER_PREVIEW_TABLE='Dialog_masterpreview.php';\r\n";
$includes.="\n</script>\n";
...
$body = array();
$body["begin"]=$includes.'
<form name="frmSearch" method="GET" action="Dialog_list.php">
...
$xt->assign("d_from_p_id_orderlinkattrs","href=\"Dialog_list.php?orderby=ad_from_p_id\"");
$xt->assign("d_from_p_id_fieldheader",true);
$xt->assign("d_to_p_id_orderlinkattrs","href=\"Dialog_list.php?orderby=ad_to_p_id\"");
$xt->assign("d_to_p_id_fieldheader",true);
$xt->assign("d_message_orderlinkattrs","href=\"Dialog_list.php?orderby=ad_message\"");
$xt->assign("d_message_fieldheader",true);
$xt->assign("d_datetime_orderlinkattrs","href=\"Dialog_list.php?orderby=ad_datetime\"");
$xt->assign("d_datetime_fieldheader",true);
$xt->assign("d_msg_type_orderlinkattrs","href=\"Dialog_list.php?orderby=ad_msg_type\"");
$xt->assign("d_msg_type_fieldheader",true);
...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	$xt->assign(GoodFieldName($order_field)."_orderlinkattrs","href=\"Dialog_list.php?orderby=".$dir.GoodFieldName($order_field)."\"");
...
if (@$_REQUEST["mdelete"])
{
	...
	{
		$keys=array();
		$keys["d_id"]=refine($_REQUEST["mdelete1"][$ind-1]);
		$selected_recs[]=$keys;
	}
}
...
{  
	...
	{
		$strWhere = "1=0";
		if($where=StrWhere("d_from_p_id", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("d_to_p_id", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("d_message", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("d_datetime", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("d_msg_type", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
...
{
	$where ="";
		$where.= GetFullFieldName("d_from_p_id")."=".make_db_value("d_from_p_id",$_SESSION[$strTableName."_masterkey1"]);
	$strWhereClause = whereAdd($strWhereClause,$where);
}
...
//	write pagination
	if($maxpages>1)
	{
		...
		{
			window.location='Dialog_list.php?goto='+nPageNumber;
		}
		</script>");
	}
...
//	add inline add row	
		$row=array();
		$row["rowattrs"]="class=\"addarea\" rowid=\"add\"";
		$row["rowspace_attrs"]="class=\"addarea\"";
		$record=array();
		...
		$record["checkbox"]=true;
		$record["editlink_attrs"]="id=\"editlink_add\"";
		$record["inlineeditlink_attrs"]= "id=\"ieditlink_add\"";
		$record["copylink_attrs"]="id=\"copylink_add\"";
		$record["viewlink_attrs"]="id=\"viewlink_add\"";
		$record["checkbox_attrs"]="id=\"checklink_add\" name=\"selection[]\"";
		$record["d_datetime_value"] = "<span id=\"add_d_datetime\">&nbsp;</span>";
		$record["d_msg_type_value"] = "<span id=\"add_d_msg_type\">&nbsp;</span>";
		$record["d_from_p_id_value"] = "<span id=\"add_d_from_p_id\">&nbsp;</span>";
		$record["d_to_p_id_value"] = "<span id=\"add_d_to_p_id\">&nbsp;</span>";
		$record["d_message_value"] = "<span id=\"add_d_message\">&nbsp;</span>";
		if($colsonpage>1)
...
//	key fields
	...
	$copylink="";
	$keylink="";
	$keyblock.= rawurlencode($data["d_id"]);
	$editlink.="editid1=".htmlspecialchars(rawurlencode($data["d_id"]));
	$copylink.="copyid1=".htmlspecialchars(rawurlencode($data["d_id"]));
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["d_id"]));

	$record["editlink_attrs"]="href=\"Dialog_edit.php?".$editlink."\" id=\"editlink".$recno."\"";
	$record["inlineeditlink_attrs"]= "href=\"Dialog_edit.php?".$editlink."\" onclick=\"return inlineEdit('".$recno."','".$editlink."');\" id=\"ieditlink".$recno."\"";
	$record["copylink_attrs"]="href=\"Dialog_add.php?".$copylink."\" id=\"copylink".$recno."\"";
	$record["viewlink_attrs"]="href=\"Dialog_view.php?".$editlink."\" id=\"viewlink".$recno."\"";
	$record["checkbox_attrs"]="name=\"selection[]\" value=\"".$keyblock."\" id=\"check".$recno."\"";


//	d_datetime - Datetime
			$value="";
				$value = ProcessLargeText(GetData($data,"d_datetime", "Datetime"),"field=d%5Fdatetime".$keylink,"",MODE_LIST);
			$record["d_datetime_value"]=$value;

//	d_msg_type - 
			$value="";
				$value = ProcessLargeText(GetData($data,"d_msg_type", ""),"field=d%5Fmsg%5Ftype".$keylink,"",MODE_LIST);
			$record["d_msg_type_value"]=$value;

//	d_from_p_id - 
			$value="";
				$value=DisplayLookupWizard("d_from_p_id",$data["d_from_p_id"],$data,$keylink,MODE_LIST);
			$record["d_from_p_id_value"]=$value;

//	d_to_p_id - 
			$value="";
				$value=DisplayLookupWizard("d_to_p_id",$data["d_to_p_id"],$data,$keylink,MODE_LIST);
			$record["d_to_p_id_value"]=$value;

//	d_message - 
			$value="";
				$value = ProcessLargeText(GetData($data,"d_message", ""),"field=d%5Fmessage".$keylink,"",MODE_LIST);
			$record["d_message_value"]=$value;
...
$xt->assign("logoutlink_attrs","onclick=\"window.location.href='login.php?a=logout';\"");
	...
	$xt->assign("printlink_attrs","href=\"Dialog_print.php\" onclick=\"window.open('Dialog_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"Dialog_print.php?all=1\" onclick=\"window.open('Dialog_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"Dialog_export.php\" onclick=\"window.open('Dialog_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='Dialog_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='Dialog_list.php'; 
	document.forms.frmAdmin.target='_self';return false\"
	href=\"Dialog_print.php\"");
	$xt->assign("exportselected_link",$allow_export);
	$xt->assign("exportselectedlink_attrs","disptype=\"control1\" onclick=\"
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='Dialog_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='Dialog_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	href=\"Dialog_export.php\"");
	
	$xt->assign("add_link",$allow_add);
	$xt->assign("copy_column",$allow_add);
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("addlink_attrs","href=\"Dialog_add.php\" onClick=\"window.location.href='Dialog_add.php'\"");
	$xt->assign("inlineaddlink_attrs","href=\"Dialog_add.php\" onclick=\"return inlineAdd(newrecord_id++);\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"Dialog_edit.php\" disptype=\"control1\" name=\"edit_selected\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
...
$xt->assign("d_datetime_fieldheadercolumn",true);
$xt->assign("d_datetime_fieldcolumn",true);
$xt->assign("d_datetime_fieldfootercolumn",true);
$xt->assign("d_msg_type_fieldheadercolumn",true);
$xt->assign("d_msg_type_fieldcolumn",true);
$xt->assign("d_msg_type_fieldfootercolumn",true);
$xt->assign("d_from_p_id_fieldheadercolumn",true);
$xt->assign("d_from_p_id_fieldcolumn",true);
$xt->assign("d_from_p_id_fieldfootercolumn",true);
$xt->assign("d_to_p_id_fieldheadercolumn",true);
$xt->assign("d_to_p_id_fieldcolumn",true);
$xt->assign("d_to_p_id_fieldfootercolumn",true);
$xt->assign("d_message_fieldheadercolumn",true);
$xt->assign("d_message_fieldcolumn",true);
$xt->assign("d_message_fieldfootercolumn",true);
	
$display_grid = $allow_add || $allow_search && $rowsfound;

$xt->assign("asearch_link",$allow_search);
$xt->assign("asearchlink_attrs","href=\"Dialog_search.php\" onclick=\"window.location.href='Dialog_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"Dialog_import.php\" onclick=\"window.location.href='Dialog_import.php';return false;\"");
...
if($display_grid)
{
	...
	"<form method=\"POST\" action=\"Dialog_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
...
$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='Dialog_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
