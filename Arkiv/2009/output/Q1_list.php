...
$xt->assign("langselector_attrs","name=lang onchange=\"javascript: window.location='Q1_list.php?language='+this.options[this.selectedIndex].value\"");
...
"\nvar bLoading=false;\r\n";
	...
	$includes.="var INLINE_EDIT_TABLE='Q1_edit.php';\r\n";
	$includes.="var INLINE_ADD_TABLE='Q1_add.php';\r\n";
	$includes.="var INLINE_VIEW_TABLE='Q1_view.php';\r\n";
	$includes.="var SUGGEST_TABLE='Q1_searchsuggest.php';\r\n";
	$includes.="var MASTER_PREVIEW_TABLE='Q1_masterpreview.php';\r\n";
$includes.="\n</script>\n";
...
$body = array();
$body["begin"]=$includes.'
<form name="frmSearch" method="GET" action="Q1_list.php">
...
$xt->assign("Q1_id_orderlinkattrs","href=\"Q1_list.php?orderby=aQ1_id\"");
$xt->assign("Q1_id_fieldheader",true);
$xt->assign("Q1_rating_orderlinkattrs","href=\"Q1_list.php?orderby=aQ1_rating\"");
$xt->assign("Q1_rating_fieldheader",true);
$xt->assign("Q1_p_id_orderlinkattrs","href=\"Q1_list.php?orderby=aQ1_p_id\"");
...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	$xt->assign(GoodFieldName($order_field)."_orderlinkattrs","href=\"Q1_list.php?orderby=".$dir.GoodFieldName($order_field)."\"");
...
{  
	...
	{
		$strWhere = "1=0";
		if($where=StrWhere("Q1_id", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Q1_work", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Q1_where", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Q1_why", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Q1_rating", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Q1_speakothers", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Q1_changedme", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Q1_playoften", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Q1_infolevel", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Q1_SNotice", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Q1_SNoticewhere", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Q1_SNoticemainsponsor", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Q1_SNoticeteam", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Q1_SNoticeother", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Q1_Shavechangedview", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Q1_Sbuyproducts", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Q1_Snameothers", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Q1_Comments", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Q1_p_id", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
...
//	write pagination
	if($maxpages>1)
	{
		...
		{
			window.location='Q1_list.php?goto='+nPageNumber;
		}
		</script>");
	}
...
//	key fields
	...
	$copylink="";
	$keylink="";
	$keyblock.= rawurlencode($data["Q1_id"]);
	$editlink.="editid1=".htmlspecialchars(rawurlencode($data["Q1_id"]));
	$copylink.="copyid1=".htmlspecialchars(rawurlencode($data["Q1_id"]));
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["Q1_id"]));

	$record["editlink_attrs"]="href=\"Q1_edit.php?".$editlink."\" id=\"editlink".$recno."\"";
	$record["inlineeditlink_attrs"]= "href=\"Q1_edit.php?".$editlink."\" onclick=\"return inlineEdit('".$recno."','".$editlink."');\" id=\"ieditlink".$recno."\"";
	$record["copylink_attrs"]="href=\"Q1_add.php?".$copylink."\" id=\"copylink".$recno."\"";
	$record["viewlink_attrs"]="href=\"Q1_view.php?".$editlink."\" id=\"viewlink".$recno."\"";
	$record["checkbox_attrs"]="name=\"selection[]\" value=\"".$keyblock."\" id=\"check".$recno."\"";


//	Q1_id - 
			$value="";
				$value = ProcessLargeText(GetData($data,"Q1_id", ""),"field=Q1%5Fid".$keylink,"",MODE_LIST);
			$record["Q1_id_value"]=$value;

//	Q1_rating - 
			$value="";
				$value = ProcessLargeText(GetData($data,"Q1_rating", ""),"field=Q1%5Frating".$keylink,"",MODE_LIST);
			$record["Q1_rating_value"]=$value;

//	Q1_p_id - 
			$value="";
				$value = ProcessLargeText(GetData($data,"Q1_p_id", ""),"field=Q1%5Fp%5Fid".$keylink,"",MODE_LIST);
...
$xt->assign("logoutlink_attrs","onclick=\"window.location.href='login.php?a=logout';\"");
	...
	$xt->assign("printlink_attrs","href=\"Q1_print.php\" onclick=\"window.open('Q1_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"Q1_print.php?all=1\" onclick=\"window.open('Q1_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"Q1_export.php\" onclick=\"window.open('Q1_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='Q1_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='Q1_list.php'; 
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='Q1_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='Q1_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	...
	$xt->assign("addlink_attrs","href=\"Q1_add.php\" onClick=\"window.location.href='Q1_add.php'\"");
	$xt->assign("inlineaddlink_attrs","href=\"Q1_add.php\" onclick=\"return inlineAdd(newrecord_id++);\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"Q1_edit.php\" disptype=\"control1\" name=\"edit_selected\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
...
$xt->assign("asearchlink_attrs","href=\"Q1_search.php\" onclick=\"window.location.href='Q1_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"Q1_import.php\" onclick=\"window.location.href='Q1_import.php';return false;\"");
...
if($display_grid)
{
	...
	"<form method=\"POST\" action=\"Q1_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
...
$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='Q1_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
