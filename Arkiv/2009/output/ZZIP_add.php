...
if($inlineedit==ADD_INLINE)
	$templatefile = "ZZIP_inline_add.htm";
else
	$templatefile = "ZZIP_add.htm";
...
if($inlineedit==ADD_INLINE || $inlineedit==ADD_ONTHEFLY || function_exists("AfterAdd"))
{
	...
	$failed_inline_add = false;
		if(array_key_exists("ZZIP",$avalues))
		$keys["ZZIP"]=$avalues["ZZIP"];
	else
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["ZZIP"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["ZZIP"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["ZZCity"]=@$avalues["ZZCity"];
}
...
//		$bodyonload="onload=\"".$bodyonload."\"";
	...
	if($inlineedit!=ADD_ONTHEFLY)
	{
		...
		$body["begin"]=$includes.
		"<form name=\"editform\" encType=\"multipart/form-data\" method=\"post\" action=\"ZZIP_add.php\" ".$onsubmit.">".
	...
	else
	{
		...
		$body["begin"]="<form name=\"editform".postvalue("id")."\" encType=\"multipart/form-data\" method=\"post\" action=\"ZZIP_add.php\" ".$onsubmit.">".
		"<input type=hidden name=\"a\" value=\"added\">".
...
if ( @$_POST["a"]=="added" && $inlineedit==ADD_INLINE ) 
{
	...
	$showKeys[] = htmlspecialchars($keys["ZZIP"]);
...
$control_ZZCity["params"]["value"]=@$defvalues["ZZCity"];
$control_ZZCity["params"]["id"]=$record_id;
if($inlineedit==ADD_INLINE)
	$control_ZZCity["params"]["mode"]="inline_add";
else
	$control_ZZCity["params"]["mode"]="add";
