...
$xt->assign("langselector_attrs","name=lang onchange=\"javascript: window.location='ZZIP_list.php?language='+this.options[this.selectedIndex].value\"");
...
//	validation stuff
	...
	$includes.="<script type=\"text/javascript\" src=\"include/inlineedit.js\"></script>\r\n";
			$editValidateTypes[] = "";
		$editValidateFields[] = "ZZCity";
	
			$addValidateTypes[] = "";
		$addValidateFields[] = "ZZCity";
...
"\nvar bLoading=false;\r\n";
	...
	$includes.="var INLINE_EDIT_TABLE='ZZIP_edit.php';\r\n";
	$includes.="var INLINE_ADD_TABLE='ZZIP_add.php';\r\n";
	$includes.="var INLINE_VIEW_TABLE='ZZIP_view.php';\r\n";
	$includes.="var SUGGEST_TABLE='ZZIP_searchsuggest.php';\r\n";
	$includes.="var MASTER_PREVIEW_TABLE='ZZIP_masterpreview.php';\r\n";
$includes.="\n</script>\n";
...
$body = array();
$body["begin"]=$includes.'
<form name="frmSearch" method="GET" action="ZZIP_list.php">
...
$xt->assign("ZZIP_orderlinkattrs","href=\"ZZIP_list.php?orderby=aZZIP\"");
$xt->assign("ZZIP_fieldheader",true);
$xt->assign("ZZCity_orderlinkattrs","href=\"ZZIP_list.php?orderby=aZZCity\"");
$xt->assign("ZZCity_fieldheader",true);
...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	$xt->assign(GoodFieldName($order_field)."_orderlinkattrs","href=\"ZZIP_list.php?orderby=".$dir.GoodFieldName($order_field)."\"");
...
if (@$_REQUEST["mdelete"])
{
	...
	{
		$keys=array();
		$keys["ZZIP"]=refine($_REQUEST["mdelete1"][$ind-1]);
		$selected_recs[]=$keys;
	}
}
...
{  
	...
	{
		$strWhere = "1=0";
		if($where=StrWhere("ZZIP", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("ZZCity", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
...
//	write pagination
	if($maxpages>1)
	{
		...
		{
			window.location='ZZIP_list.php?goto='+nPageNumber;
		}
		</script>");
	}
...
//	add inline add row	
		...
		$record["ZZIP_value"] = "<span id=\"add_ZZIP\">&nbsp;</span>";
		$record["ZZCity_value"] = "<span id=\"add_ZZCity\">&nbsp;</span>";
		if($colsonpage>1)
...
//	key fields
	...
	$copylink="";
	$keylink="";
	$keyblock.= rawurlencode($data["ZZIP"]);
	$editlink.="editid1=".htmlspecialchars(rawurlencode($data["ZZIP"]));
	$copylink.="copyid1=".htmlspecialchars(rawurlencode($data["ZZIP"]));
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["ZZIP"]));

	$record["editlink_attrs"]="href=\"ZZIP_edit.php?".$editlink."\" id=\"editlink".$recno."\"";
	$record["inlineeditlink_attrs"]= "href=\"ZZIP_edit.php?".$editlink."\" onclick=\"return inlineEdit('".$recno."','".$editlink."');\" id=\"ieditlink".$recno."\"";
	$record["copylink_attrs"]="href=\"ZZIP_add.php?".$copylink."\" id=\"copylink".$recno."\"";
	$record["viewlink_attrs"]="href=\"ZZIP_view.php?".$editlink."\" id=\"viewlink".$recno."\"";
	$record["checkbox_attrs"]="name=\"selection[]\" value=\"".$keyblock."\" id=\"check".$recno."\"";


//	ZZIP - 
			$value="";
				$value = ProcessLargeText(GetData($data,"ZZIP", ""),"field=ZZIP".$keylink,"",MODE_LIST);
			$record["ZZIP_value"]=$value;

//	ZZCity - 
			$value="";
				$value = ProcessLargeText(GetData($data,"ZZCity", ""),"field=ZZCity".$keylink,"",MODE_LIST);
			$record["ZZCity_value"]=$value;
			if(function_exists("BeforeMoveNextList"))
				BeforeMoveNextList($data,$record,$col);
			$span="<span ";
			$span.="id=\"edit".$recno."_ZZIP\" ";
					$span.=">";
			$record["ZZIP_value"] = $span.$record["ZZIP_value"]."</span>";
			$span="<span ";
			$span.="id=\"edit".$recno."_ZZCity\" ";
					$span.=">";
			$record["ZZCity_value"] = $span.$record["ZZCity_value"]."</span>";
			if($col<$colsonpage)
				$record["endrecord_block"]=true;
			$record["grid_recordheader"]=true;
			$record["grid_vrecord"]=true;
...
$xt->assign("logoutlink_attrs","onclick=\"window.location.href='login.php?a=logout';\"");
	...
	$xt->assign("printlink_attrs","href=\"ZZIP_print.php\" onclick=\"window.open('ZZIP_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"ZZIP_print.php?all=1\" onclick=\"window.open('ZZIP_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"ZZIP_export.php\" onclick=\"window.open('ZZIP_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='ZZIP_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='ZZIP_list.php'; 
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='ZZIP_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='ZZIP_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	href=\"ZZIP_export.php\"");
	
	$xt->assign("add_link",$allow_add);
	$xt->assign("copy_column",$allow_add);
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("addlink_attrs","href=\"ZZIP_add.php\" onClick=\"window.location.href='ZZIP_add.php'\"");
	$xt->assign("inlineaddlink_attrs","href=\"ZZIP_add.php\" onclick=\"return inlineAdd(newrecord_id++);\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"ZZIP_edit.php\" disptype=\"control1\" name=\"edit_selected\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
...
$xt->assign("ZZIP_fieldheadercolumn",true);
$xt->assign("ZZIP_fieldcolumn",true);
$xt->assign("ZZIP_fieldfootercolumn",true);
$xt->assign("ZZCity_fieldheadercolumn",true);
$xt->assign("ZZCity_fieldcolumn",true);
$xt->assign("ZZCity_fieldfootercolumn",true);
	
$display_grid = $allow_add || $allow_search && $rowsfound;

$xt->assign("asearch_link",$allow_search);
$xt->assign("asearchlink_attrs","href=\"ZZIP_search.php\" onclick=\"window.location.href='ZZIP_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"ZZIP_import.php\" onclick=\"window.location.href='ZZIP_import.php';return false;\"");
...
if($display_grid)
{
	...
	"<form method=\"POST\" action=\"ZZIP_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
...
$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='ZZIP_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
