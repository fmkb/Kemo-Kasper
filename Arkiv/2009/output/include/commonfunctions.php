...
function GetTableURL($table="")
{
	...
	if("MQ Chart"==$table) 
		return "MQ_Chart";
	if("Accept news Chart"==$table) 
		return "Accept_news_Chart";
	if("xcountries"==$table) 
		return "xcountries";
}
...
{
	if($shorttable=="MQ_Chart")
		return "2DPie";
	if($shorttable=="Accept_news_Chart")
		return "2DPie";
	return "";
}
...
// long binary data?
	...
	else if($format == FORMAT_FILE_IMAGE)
	{
		...
		$thumbnailed=false;
		$thumbprefix="";
		if($strTableName=="news" && $field=="n_file")
		{
			$thumbnailed=true;
			$thumbprefix="th_";
		}
	...
	else if($format==FORMAT_FILE)
	{
		...
		if($strTableName=="Dialog")
		{
			$iquery.="&key1=".rawurlencode($data["d_id"]);
		}
		if($strTableName=="mails")
		{
			$iquery.="&key1=".rawurlencode($data["m_id"]);
		}
		if($strTableName=="news")
		{
			$iquery.="&key1=".rawurlencode($data["n_id"]);
		}
		if($strTableName=="player")
		{
			$iquery.="&key1=".rawurlencode($data["p_id"]);
		}
		if($strTableName=="raffle")
		{
			$iquery.="&key1=".rawurlencode($data["r_id"]);
		}
		if($strTableName=="sponsor")
		{
			$iquery.="&key1=".rawurlencode($data["s_id"]);
		}
		if($strTableName=="top")
		{
			$iquery.="&key1=".rawurlencode($data["t_id"]);
		}
		if($strTableName=="teams")
		{
			$iquery.="&key1=".rawurlencode($data["ts_id"]);
		}
		if($strTableName=="MQ Chart")
		{
		}
		if($strTableName=="Accept news Chart")
		{
		}
		if($strTableName=="xcountries")
		{
			$iquery.="&key1=".rawurlencode($data["xc_id"]);
		}
...
//		$ret.="pwin.document.write('" . db_addslashes(str_replace("\r\n","<br>",htmlspecialchars(substr($strValue,0, 801)))) ."');";
		$ret.="pwin.document.write('<br><hr size=1 noshade><a href=# onClick=\\'window.close();return false;\\'>".mlang_message("CLOSE_WINDOW")."</a>');";
		$ret.="return false;\">".mlang_message("MORE")." ...</a>";
	}
	else if($cNumberOfChars>0 && strlen($strValue)>$cNumberOfChars && $mode==MODE_LIST)
	{
		...
		$ret=htmlspecialchars($ret);
		$ret.=" <a href=#  onClick=\"javascript: pwin = window.open('',null,'height=300,width=400,status=yes,resizable=yes,toolbar=no,menubar=no,location=no,left=150,top=200,scrollbars=yes');";
		$ret.=" pwin.location='".$table."_fulltext.php?".$iquery."'; return false;\">".mlang_message("MORE")." ...</a>";
	}
...
<p align=center><font size=+2>PHP <?php echo mlang_message("ERR_HAPPENED");?></font></p>
<table border="0" cellpadding="3" cellspacing="1" width="700" bgcolor="#000000" align="center">
<tr><td bgcolor="#ccccff" colspan=2 align=middle><font size=+1><b><?php echo mlang_message("TECH_INFO");?></b></font></td></tr>
<tr bgcolor="#cccccc"><td bgcolor="#ccccff"><b><?php echo mlang_message("ERR_TYPE");?></b></td><td align="left"><?php echo $errno; ?></td></tr>
<tr bgcolor="#cccccc"><td bgcolor="#ccccff"><b><?php echo mlang_message("ERR_DESC");?></b></td><td align="left"><font color=#cc3300><?php echo $errstr?></font></td></tr>
...
<tr bgcolor="#cccccc"><td bgcolor="#ccccff"><b><?php echo mlang_message("ERR_FILE");?></b></td><td align="left"><?php echo $errfile;?></td></tr>
<tr bgcolor="#cccccc"><td bgcolor="#ccccff"><b><?php echo mlang_message("ERR_LINE");?></b></td><td align="left"><?php echo $errline;?></td></tr>
<tr bgcolor="#cccccc"><td bgcolor="#ccccff" ><b><?php echo mlang_message("SQL_QUERY");?></b></td><td align="left"><?php if(isset($strSQL)) echo substr($strSQL,0,1024);?></td></tr>
<?php if ($solution) 
{?>
...
//	Dialog
	if($table=="Dialog")
	{
			$value=make_db_value("d_id",$keys["d_id"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("d_id")." is null";
		else
			$strWhere.=GetFullFieldName("d_id")."=".make_db_value("d_id",$keys["d_id"]);
	}

//	mails
	if($table=="mails")
	{
			$value=make_db_value("m_id",$keys["m_id"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("m_id")." is null";
		else
			$strWhere.=GetFullFieldName("m_id")."=".make_db_value("m_id",$keys["m_id"]);
	}

//	news
	if($table=="news")
	{
			$value=make_db_value("n_id",$keys["n_id"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("n_id")." is null";
		else
			$strWhere.=GetFullFieldName("n_id")."=".make_db_value("n_id",$keys["n_id"]);
	}

//	player
	if($table=="player")
	{
			$value=make_db_value("p_id",$keys["p_id"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("p_id")." is null";
		else
			$strWhere.=GetFullFieldName("p_id")."=".make_db_value("p_id",$keys["p_id"]);
	}

//	raffle
	if($table=="raffle")
	{
			$value=make_db_value("r_id",$keys["r_id"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("r_id")." is null";
		else
			$strWhere.=GetFullFieldName("r_id")."=".make_db_value("r_id",$keys["r_id"]);
	}

//	sponsor
	if($table=="sponsor")
	{
			$value=make_db_value("s_id",$keys["s_id"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("s_id")." is null";
		else
			$strWhere.=GetFullFieldName("s_id")."=".make_db_value("s_id",$keys["s_id"]);
	}

//	top
	if($table=="top")
	{
			$value=make_db_value("t_id",$keys["t_id"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("t_id")." is null";
		else
			$strWhere.=GetFullFieldName("t_id")."=".make_db_value("t_id",$keys["t_id"]);
	}

//	teams
	if($table=="teams")
	{
			$value=make_db_value("ts_id",$keys["ts_id"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("ts_id")." is null";
		else
			$strWhere.=GetFullFieldName("ts_id")."=".make_db_value("ts_id",$keys["ts_id"]);
	}
...
//	xcountries
	if($table=="xcountries")
	{
			$value=make_db_value("xc_id",$keys["xc_id"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("xc_id")." is null";
		else
			$strWhere.=GetFullFieldName("xc_id")."=".make_db_value("xc_id",$keys["xc_id"]);
	}
	return $strWhere;
}
...
{
	$monthnames=array();
	$monthnames[1]=mlang_message("MONTH_JAN");
	$monthnames[2]=mlang_message("MONTH_FEB");
	$monthnames[3]=mlang_message("MONTH_MAR");
	$monthnames[4]=mlang_message("MONTH_APR");
	$monthnames[5]=mlang_message("MONTH_MAY");
	$monthnames[6]=mlang_message("MONTH_JUN");
	$monthnames[7]=mlang_message("MONTH_JUL");
	$monthnames[8]=mlang_message("MONTH_AUG");
	$monthnames[9]=mlang_message("MONTH_SEP");
	$monthnames[10]=mlang_message("MONTH_OCT");
	$monthnames[11]=mlang_message("MONTH_NOV");
	$monthnames[12]=mlang_message("MONTH_DEC");
	$ret='<option value=""></option>';
	for($i=1;$i<=12;$i++)
		...
		case EDIT_DATE_SIMPLE_DP:
			...
			if ( $inline ) {
				$ret.='<a href="#" onclick="javascript:var v=show_calendar(\'update\', \''.$iname.'\',\''.$record_id.'\', $(\'input#ts'.$iname.'_'.$record_id.'\').get(0).value,'.$showtime.','.$sundayfirst.'); return false;">'.
					'<img src="images/cal.gif" width=16 height=16 border=0 alt="'.mlang_message("PICKUP_DATE").'"></a>';
...
//	build SQL strings for lookup wizards
		if($strTableName=="Dialog" && $field=="d_from_p_id") 
		{
				$LinkField="p_id";
			$DisplayField="concat(P_First,' ',P_Name)";
			$LookupTable="player";
			$strSize=1;
			...
			$LookupSQL = "select ";
						...
						$LookupSQL.=" ORDER BY `player`.`p_name`";
						$LookupSQL.=" DESC";

		}
		if($strTableName=="Dialog" && $field=="d_to_p_id") 
		{
				$LinkField="p_id";
			$DisplayField="concat(P_First,' ',P_Name)";
			$LookupTable="player";
		...
		if($strTableName=="news" && $field=="n_active") 
		{
				$LinkField="xs_id";
			$DisplayField="xs_text";
			$LookupTable="xstatus";
			$strSize=1;
		...
		if($strTableName=="news" && $field=="n_country") 
		{
				$LinkField="xc_id";
			$DisplayField="xc_name";
			$LookupTable="xcountries";
			$strSize=1;
		...
		if($strTableName=="player" && $field=="p_active") 
		{
				$LinkField="xs_id";
			$DisplayField="xs_text";
			$LookupTable="xstatus";
		...
		if($strTableName=="player" && $field=="p_s_id") 
		{
				$LinkField="ts_id";
			$DisplayField="concat('(',ts_id,') ',ts_name)";
			$LookupTable="teams";
			$strSize=1;
			...
			$LookupSQL = "select ";
						...
						$LookupSQL.=" ORDER BY `teams`.`ts_id`";
			
		}
		if($strTableName=="raffle" && $field=="r_p_id") 
		{
				$LinkField="p_id";
			$DisplayField="p_mail";
			$LookupTable="player";
		...
		if($strTableName=="sponsor" && $field=="s_active") 
		{
				$LinkField="xs_id";
			$DisplayField="xs_text";
			$LookupTable="xstatus";
		...
		if($strTableName=="sponsor" && $field=="s_country") 
		{
				$LinkField="xc_id";
			$DisplayField="xc_name";
			$LookupTable="xcountries";
			...
			$LookupSQL = "select ";
						...
						$LookupSQL.=" ORDER BY `xcountries`.`xc_id`";
			
		}
		if($strTableName=="top" && $field=="t_p_id") 
		{
				$LinkField="p_id";
			$DisplayField="p_mail";
			$LookupTable="player";
		...
		if($strTableName=="teams" && $field=="ts_s_id") 
		{
				$LinkField="s_id";
			$DisplayField="s_name";
			$LookupTable="sponsor";
...
//	get the initial value
			...
			if($strTableName=="Dialog" && $field=="d_from_p_id") 
			{
								$lookup_SQL = "SELECT ";
								$lookup_SQL .= "`p_id`";
								$lookup_SQL .= ",concat(P_First,' ',P_Name)";
				$lookup_SQL .= " FROM `player` ";
								$lookup_SQL .= " WHERE `p_id`=".make_db_value($field,$value)."";
									$lookup_SQL.= " ORDER BY `p_name`";
										$lookup_SQL.=" DESC";	
			}
			if($strTableName=="Dialog" && $field=="d_to_p_id") 
			{
			...
			if($strTableName=="news" && $field=="n_active") 
			{
								...
								$lookup_SQL .= "`xs_id`";
								$lookup_SQL .= ",`xs_text`";
				$lookup_SQL .= " FROM `xstatus` ";
								$lookup_SQL .= " WHERE `xs_id`=".make_db_value($field,$value)."";
							}
			if($strTableName=="news" && $field=="n_country") 
			{
								$lookup_SQL = "SELECT ";
								$lookup_SQL .= "`xc_id`";
								$lookup_SQL .= ",`xc_name`";
				$lookup_SQL .= " FROM `xcountries` ";
								$lookup_SQL .= " WHERE `xc_id`=".make_db_value($field,$value)."";
							}
			if($strTableName=="player" && $field=="p_active") 
			{
								$lookup_SQL = "SELECT ";
			...
			if($strTableName=="player" && $field=="p_s_id") 
			{
								$lookup_SQL = "SELECT ";
								$lookup_SQL .= "`ts_id`";
								$lookup_SQL .= ",concat('(',ts_id,') ',ts_name)";
				$lookup_SQL .= " FROM `teams` ";
								$lookup_SQL .= " WHERE `ts_id`=".make_db_value($field,$value)."";
									$lookup_SQL.= " ORDER BY `ts_id`";
								}
			if($strTableName=="raffle" && $field=="r_p_id") 
			{
								$lookup_SQL = "SELECT ";
			...
			if($strTableName=="teams" && $field=="ts_s_id") 
			{
								...
								$lookup_SQL .= "`s_id`";
								$lookup_SQL .= ",`s_name`";
				$lookup_SQL .= " FROM `sponsor` ";
								$lookup_SQL .= " WHERE `s_id`=".make_db_value($field,$value)."";
							}
...
//	build the control
			...
			if($addnewitem &&  $mode!=MODE_SEARCH)
			{
				if(!$advancedadd)
				{
					...
					mlang_message("ADD_NEW")."</a>";
				}
				else
				{
					if( $mode==MODE_INLINE_EDIT || $mode==MODE_INLINE_ADD )
						echo "<a href=# onclick=\"return DisplayAdd(event,'".$add_page."','value_".GoodFieldName($field)."_".$id."','".GoodFieldName($field)."','".GetTableURL()."','');\">".mlang_message("ADD_NEW")."</a>";
					else
						echo "<a href=# onclick=\"return DisplayAdd(event,'".$add_page."','value_".GoodFieldName($field)."','".GoodFieldName($field)."','".GetTableURL()."','');\">".mlang_message("ADD_NEW")."</a>";
				}
			}
...
//	add new item
			if($addnewitem &&  $mode!=MODE_SEARCH && $mode!=MODE_INLINE_EDIT && $mode!=MODE_INLINE_ADD)
			{
				...
				{
					echo "<a href=# onclick=\"return DisplayAdd(event,'".$add_page."','".htmlspecialchars(jsreplace($cfield))."','".GoodFieldName($field)."','".GetTableURL()."','');\">".mlang_message("ADD_NEW")."</a>";
				}
			}
			if($addnewitem &&  $mode!=MODE_SEARCH &&  ($mode==MODE_INLINE_EDIT || $mode==MODE_INLINE_ADD))
			{
...
function BuildEditControl($field , $value, $format, $edit, $secondfield=false, $id="")
{
	...
	if($strTableName=="Dialog")
	{
		$keylink.="&key1=".rawurlencode(@$keys["d_id"]);
		$iquery.=$keylink;
	}
	if($strTableName=="mails")
	{
		$keylink.="&key1=".rawurlencode(@$keys["m_id"]);
		$iquery.=$keylink;
	}
	if($strTableName=="news")
	{
		$keylink.="&key1=".rawurlencode(@$keys["n_id"]);
		$iquery.=$keylink;
	}
	if($strTableName=="player")
	{
		$keylink.="&key1=".rawurlencode(@$keys["p_id"]);
		$iquery.=$keylink;
	}
	if($strTableName=="raffle")
	{
		$keylink.="&key1=".rawurlencode(@$keys["r_id"]);
		$iquery.=$keylink;
	}
	if($strTableName=="sponsor")
	{
		$keylink.="&key1=".rawurlencode(@$keys["s_id"]);
		$iquery.=$keylink;
	}
	if($strTableName=="top")
	{
		$keylink.="&key1=".rawurlencode(@$keys["t_id"]);
		$iquery.=$keylink;
	}
	if($strTableName=="teams")
	{
		$keylink.="&key1=".rawurlencode(@$keys["ts_id"]);
		$iquery.=$keylink;
	}
	if($strTableName=="MQ Chart")
	{
	...
	if($strTableName=="xcountries")
	{
		$keylink.="&key1=".rawurlencode(@$keys["xc_id"]);
		$iquery.=$keylink;
	}
...
//	filename edit
			if($format==EDIT_FORMAT_DATABASE_FILE && GetFilenameField($field))
			{
				...
				{
					$strfilename='<br>'.mlang_message("FILENAME").'&nbsp;&nbsp;<input id="filename_'.$cfieldname.'_'.$id.'" name="filename_'.$cfieldname.'" size="20" maxlength="50" value="'.htmlspecialchars($filename).'">';
				...
				{
					$strfilename='<br>'.mlang_message("FILENAME").'&nbsp;&nbsp;<input name="filename_'.$cfieldname.'" size="20" maxlength="50" value="'.htmlspecialchars($filename).'">';
			...
			$strtype='<br><input type="Radio" name="'.$ctype.'" value="file0" checked>'.mlang_message("KEEP");
			if(strlen($value) && !IsRequired($field))
			{
				$strtype.='<input type="Radio" name="'.$ctype.'" value="file1">'.mlang_message("DELETE");
				if ($edit==MODE_INLINE_EDIT ) {
			...
			$strtype.='<input type="Radio" name="'.$ctype.'" value="file2">'.mlang_message("UPDATE");
		}
		else
		{
//	if Add mode
			...
			if($format==EDIT_FORMAT_DATABASE_FILE && GetFilenameField($field))
			{
				$strfilename='<br>'.mlang_message("FILENAME").'&nbsp;&nbsp;<input name="filename_'.$cfieldname.'" size="20" maxlength="50">';
...
//	filename edit
			...
			if(UseTimestamp($field))
				$filename_size=50;
			$strfilename='<input type=hidden name="filename_'.$cfieldname.'" value="'.htmlspecialchars($filename).'"><br>'.mlang_message("FILENAME").'&nbsp;&nbsp;<input style="background-color:gainsboro" disabled id="'.$cfield.'_'.$id.'" name="'.$cfield.'" size="'.$filename_size.'" maxlength="100" value="'.htmlspecialchars($filename).'">';
			if ( $edit==MODE_INLINE_EDIT ) {
				...
				$strtype='<br><input type="Radio" name="'.$ctype.'" value="upload0" checked onclick="$(\'[@id='.$cfield.'_'.$id.']\').css(\'backgroundColor\',\'gainsboro\');$(\'[@id='.$cfield.'_'.$id.']\')[0].disabled=true;">'.mlang_message("KEEP");
			} else {
				...
				$strtype='<br><input type="Radio" name="'.$ctype.'" value="upload0" checked onclick="controlfilename'.$cfieldname.'(false)">'.mlang_message("KEEP");
			}
			...
			{
				if ($edit==MODE_INLINE_EDIT) {
					$strtype.='<input type="Radio" name="'.$ctype.'" value="upload1" onclick="$(\'[@id='.$cfield.'_'.$id.']\').css(\'backgroundColor\',\'gainsboro\');$(\'[@id='.$cfield.'_'.$id.']\')[0].disabled=true;">'.mlang_message("DELETE");
					$onchangefile.='$(\'input[@type=radio][@value=upload2][@name='.$ctype.']\').get(0).checked=true;';
				} else {
					$strtype.='<input type="Radio" name="'.$ctype.'" value="upload1" onclick="controlfilename'.$cfieldname.'(false)">'.mlang_message("DELETE");
			...
			if ($edit==MODE_INLINE_EDIT) {
				$strtype.='<input type="Radio" name="'.$ctype.'" value="upload2" onclick="$(\'[@id='.$cfield.'_'.$id.']\').css(\'backgroundColor\',\'white\');$(\'[@id='.$cfield.'_'.$id.']\')[0].disabled=false;">'.mlang_message("UPDATE");
			} else {
				$strtype.='<input type="Radio" name="'.$ctype.'" value="upload2" onclick="controlfilename'.$cfieldname.'(true)">'.mlang_message("UPDATE");
			}
		}
		else
		{
//	if Adding record		
			...
			$strfilename='<br>'.mlang_message("FILENAME").'&nbsp;&nbsp;<input name="'.$cfield.'" size="'.$filename_size.'" maxlength="100">';
			if($edit==MODE_INLINE_ADD)
			{
...
if ("MQ_Chart"==$chartname)
{
return '<chart>
<attr value="tables">
	<attr value="0">MQ Chart</attr>
</attr>
...
<attr value="0">
<attr value="name">count(p_mk)</attr>
</attr>
<attr value="1">
<attr value="name">p_mk</attr>
</attr>

</attr>

<attr value="appearance">
	<attr value="color11">D69367</attr>
	<attr value="color12">D69367</attr>
	<attr value="color21">B86262</attr>
	<attr value="color22">B86262</attr>
	<attr value="color31">AEC7D9</attr>
	<attr value="color32">AEC7D9</attr>
	<attr value="color41">D0AE47</attr>
	<attr value="color42">D0AE47</attr>
...
<attr value="dec">0</attr>
<attr value="head">Boys vs Girls count</attr>
...
<attr value="settings">
<attr value="name">p_mk</attr>
<attr value="short_table_name">MQ_Chart</attr>
</attr>

</chart>';
}
if ("Accept_news_Chart"==$chartname)
{
return '<chart>
<attr value="tables">
	<attr value="0">Accept news Chart</attr>
</attr>
<attr value="chart_type">
	<attr value="type">2d_pie</attr>
</attr>
...
<attr value="parameters">
<attr value="0">
<attr value="name">COUNT(p_newsaccept)</attr>
</attr>
<attr value="1">
<attr value="name">p_newsaccept</attr>
</attr>
...
<attr value="slog">false</attr>
<attr value="dec">0</attr>
<attr value="head">Accept News Chart</attr>
<attr value="foot"></attr>
<attr value="aqua">0</attr>
<attr value="cview">Column</attr>
</attr>
...
<attr value="settings">
<attr value="name">p_newsaccept</attr>
<attr value="short_table_name">Accept_news_Chart</attr>
</attr>
