...
function DisplayFlyDiv(html,js,id,control,x,y)
{
	...
	if($.browser.msie)
	 ...
	 w = 'width:inherit;';	
	$(document.body).append("<div id='flycontainer"+id+"' style='position:absolute;'><div align=center id=fly"+id+" style='"+w+"border:solid black 1px; background:white;'  control='"+control+"' onmousedown='flydivonclick(this);'></div></div>");
	...
	document.body.appendChild(flycontainer.removeChild(flydiv));
    ...
    if ( window.ActiveXObject ) {
		...
		$(io).load(function(){
			if (typeof this.loadCount == 'undefined') {
				...
				removeflyframe = this;
				setTimeout('$(removeflyframe).remove()',500);
		});
	}
