...
function SaveChart($report, $rtitle, $rstatus, $strXML) {
	...
	if ( $cnt > 0 ) {
		...
		$strSQL = "INSERT INTO webreports VALUES(NULL, '" . $report . "', '" . $rtitle . "', now(), now(), '" . $strXML . "', '" . @$_SESSION["UserID"] . "', '" . $rstatus . "', 'chart')";
		$rsReport = db_exec($strSQL,$conn);
	}
}
...
function SaveReport($report, $rtitle, $rstatus, $strXML) {
	...
	if ( $cnt > 0 ) {
		...
		$strSQL = "INSERT INTO webreports VALUES(NULL, '" . $report . "', '" . $rtitle . "', now(), now(), '" . $strXML . "', '" . @$_SESSION["UserID"] . "', '" . $rstatus . "', 'report')";
		$rsReport = db_exec($strSQL,$conn);
	}
}
