...
$xt->assign("langselector_attrs","name=lang onchange=\"javascript: window.location='mails_list.php?language='+this.options[this.selectedIndex].value\"");
...
"\nvar bLoading=false;\r\n";
	...
	$includes.="var INLINE_EDIT_TABLE='mails_edit.php';\r\n";
	$includes.="var INLINE_ADD_TABLE='mails_add.php';\r\n";
	$includes.="var INLINE_VIEW_TABLE='mails_view.php';\r\n";
	$includes.="var SUGGEST_TABLE='mails_searchsuggest.php';\r\n";
	$includes.="var MASTER_PREVIEW_TABLE='mails_masterpreview.php';\r\n";
$includes.="\n</script>\n";
...
$body = array();
$body["begin"]=$includes.'
<form name="frmSearch" method="GET" action="mails_list.php">
...
$xt->assign("m_id_orderlinkattrs","href=\"mails_list.php?orderby=am_id\"");
$xt->assign("m_id_fieldheader",true);
$xt->assign("m_name_orderlinkattrs","href=\"mails_list.php?orderby=am_name\"");
$xt->assign("m_name_fieldheader",true);
...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	$xt->assign(GoodFieldName($order_field)."_orderlinkattrs","href=\"mails_list.php?orderby=".$dir.GoodFieldName($order_field)."\"");
...
if (@$_REQUEST["mdelete"])
{
	...
	{
		$keys=array();
		$keys["m_id"]=refine($_REQUEST["mdelete1"][$ind-1]);
		$selected_recs[]=$keys;
	}
}
...
{  
	...
	{
		$strWhere = "1=0";
		if($where=StrWhere("m_id", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("m_name", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("m_body", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
...
//	write pagination
	if($maxpages>1)
	{
		...
		{
			window.location='mails_list.php?goto='+nPageNumber;
		}
		</script>");
	}
...
//	key fields
	...
	$copylink="";
	$keylink="";
	$keyblock.= rawurlencode($data["m_id"]);
	$editlink.="editid1=".htmlspecialchars(rawurlencode($data["m_id"]));
	$copylink.="copyid1=".htmlspecialchars(rawurlencode($data["m_id"]));
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["m_id"]));

	$record["editlink_attrs"]="href=\"mails_edit.php?".$editlink."\" id=\"editlink".$recno."\"";
	$record["inlineeditlink_attrs"]= "href=\"mails_edit.php?".$editlink."\" onclick=\"return inlineEdit('".$recno."','".$editlink."');\" id=\"ieditlink".$recno."\"";
	$record["copylink_attrs"]="href=\"mails_add.php?".$copylink."\" id=\"copylink".$recno."\"";
	$record["viewlink_attrs"]="href=\"mails_view.php?".$editlink."\" id=\"viewlink".$recno."\"";
	$record["checkbox_attrs"]="name=\"selection[]\" value=\"".$keyblock."\" id=\"check".$recno."\"";


//	m_id - 
			$value="";
				$value = ProcessLargeText(GetData($data,"m_id", ""),"field=m%5Fid".$keylink,"",MODE_LIST);
			$record["m_id_value"]=$value;

//	m_name - 
			$value="";
				$value = ProcessLargeText(GetData($data,"m_name", ""),"field=m%5Fname".$keylink,"",MODE_LIST);
			$record["m_name_value"]=$value;
...
$xt->assign("logoutlink_attrs","onclick=\"window.location.href='login.php?a=logout';\"");
	...
	$xt->assign("printlink_attrs","href=\"mails_print.php\" onclick=\"window.open('mails_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"mails_print.php?all=1\" onclick=\"window.open('mails_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"mails_export.php\" onclick=\"window.open('mails_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='mails_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='mails_list.php'; 
	document.forms.frmAdmin.target='_self';return false\"
	href=\"mails_print.php\"");
	$xt->assign("exportselected_link",$allow_export);
	$xt->assign("exportselectedlink_attrs","disptype=\"control1\" onclick=\"
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='mails_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='mails_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	href=\"mails_export.php\"");
	
	$xt->assign("add_link",$allow_add);
	$xt->assign("copy_column",$allow_add);
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("addlink_attrs","href=\"mails_add.php\" onClick=\"window.location.href='mails_add.php'\"");
	$xt->assign("inlineaddlink_attrs","href=\"mails_add.php\" onclick=\"return inlineAdd(newrecord_id++);\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"mails_edit.php\" disptype=\"control1\" name=\"edit_selected\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
...
$xt->assign("m_id_fieldheadercolumn",true);
$xt->assign("m_id_fieldcolumn",true);
$xt->assign("m_id_fieldfootercolumn",true);
$xt->assign("m_name_fieldheadercolumn",true);
$xt->assign("m_name_fieldcolumn",true);
$xt->assign("m_name_fieldfootercolumn",true);
	
$display_grid = $allow_search && $rowsfound;

$xt->assign("asearch_link",$allow_search);
$xt->assign("asearchlink_attrs","href=\"mails_search.php\" onclick=\"window.location.href='mails_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"mails_import.php\" onclick=\"window.location.href='mails_import.php';return false;\"");
...
if($display_grid)
{
	...
	"<form method=\"POST\" action=\"mails_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
...
$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='mails_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
