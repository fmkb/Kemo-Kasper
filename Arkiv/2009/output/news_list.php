...
$xt->assign("langselector_attrs","name=lang onchange=\"javascript: window.location='news_list.php?language='+this.options[this.selectedIndex].value\"");
...
"\nvar bLoading=false;\r\n";
	...
	$includes.="var INLINE_EDIT_TABLE='news_edit.php';\r\n";
	$includes.="var INLINE_ADD_TABLE='news_add.php';\r\n";
	$includes.="var INLINE_VIEW_TABLE='news_view.php';\r\n";
	$includes.="var SUGGEST_TABLE='news_searchsuggest.php';\r\n";
	$includes.="var MASTER_PREVIEW_TABLE='news_masterpreview.php';\r\n";
$includes.="\n</script>\n";
...
$body = array();
$body["begin"]=$includes.'
<form name="frmSearch" method="GET" action="news_list.php">
...
$xt->assign("n_id_orderlinkattrs","href=\"news_list.php?orderby=an_id\"");
$xt->assign("n_id_fieldheader",true);
$xt->assign("n_active_orderlinkattrs","href=\"news_list.php?orderby=an_active\"");
$xt->assign("n_active_fieldheader",true);
$xt->assign("n_start_orderlinkattrs","href=\"news_list.php?orderby=an_start\"");
$xt->assign("n_start_fieldheader",true);
$xt->assign("n_end_orderlinkattrs","href=\"news_list.php?orderby=an_end\"");
$xt->assign("n_end_fieldheader",true);
$xt->assign("n_date_orderlinkattrs","href=\"news_list.php?orderby=an_date\"");
$xt->assign("n_date_fieldheader",true);
$xt->assign("n_head_orderlinkattrs","href=\"news_list.php?orderby=an_head\"");
$xt->assign("n_head_fieldheader",true);
$xt->assign("n_file_orderlinkattrs","href=\"news_list.php?orderby=an_file\"");
$xt->assign("n_file_fieldheader",true);
$xt->assign("n_type_orderlinkattrs","href=\"news_list.php?orderby=an_type\"");
$xt->assign("n_type_fieldheader",true);
$xt->assign("n_teaser_orderlinkattrs","href=\"news_list.php?orderby=an_teaser\"");
$xt->assign("n_teaser_fieldheader",true);
$xt->assign("n_country_orderlinkattrs","href=\"news_list.php?orderby=an_country\"");
$xt->assign("n_country_fieldheader",true);
...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	$xt->assign(GoodFieldName($order_field)."_orderlinkattrs","href=\"news_list.php?orderby=".$dir.GoodFieldName($order_field)."\"");
...
{  
	...
	{
		$strWhere = "1=0";
		if($where=StrWhere("n_id", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("n_active", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("n_start", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("n_end", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("n_date", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("n_head", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("n_text", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("n_file", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("n_type", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("n_teaser", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("n_country", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
...
//	write pagination
	if($maxpages>1)
	{
		...
		{
			window.location='news_list.php?goto='+nPageNumber;
		}
		</script>");
	}
...
//	key fields
	...
	$copylink="";
	$keylink="";
	$keyblock.= rawurlencode($data["n_id"]);
	$editlink.="editid1=".htmlspecialchars(rawurlencode($data["n_id"]));
	$copylink.="copyid1=".htmlspecialchars(rawurlencode($data["n_id"]));
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["n_id"]));

	$record["editlink_attrs"]="href=\"news_edit.php?".$editlink."\" id=\"editlink".$recno."\"";
	$record["inlineeditlink_attrs"]= "href=\"news_edit.php?".$editlink."\" onclick=\"return inlineEdit('".$recno."','".$editlink."');\" id=\"ieditlink".$recno."\"";
	$record["copylink_attrs"]="href=\"news_add.php?".$copylink."\" id=\"copylink".$recno."\"";
	$record["viewlink_attrs"]="href=\"news_view.php?".$editlink."\" id=\"viewlink".$recno."\"";
	$record["checkbox_attrs"]="name=\"selection[]\" value=\"".$keyblock."\" id=\"check".$recno."\"";


//	n_id - 
			$value="";
				$value = ProcessLargeText(GetData($data,"n_id", ""),"field=n%5Fid".$keylink,"",MODE_LIST);
			$record["n_id_value"]=$value;

//	n_active - 
			$value="";
				$value=DisplayLookupWizard("n_active",$data["n_active"],$data,$keylink,MODE_LIST);
			$record["n_active_value"]=$value;

//	n_start - Short Date
			$value="";
				$value = ProcessLargeText(GetData($data,"n_start", "Short Date"),"field=n%5Fstart".$keylink,"",MODE_LIST);
			$record["n_start_value"]=$value;

//	n_end - Short Date
			$value="";
				$value = ProcessLargeText(GetData($data,"n_end", "Short Date"),"field=n%5Fend".$keylink,"",MODE_LIST);
			$record["n_end_value"]=$value;

//	n_date - Short Date
			$value="";
				$value = ProcessLargeText(GetData($data,"n_date", "Short Date"),"field=n%5Fdate".$keylink,"",MODE_LIST);
			$record["n_date_value"]=$value;

//	n_head - 
			$value="";
				$value = ProcessLargeText(GetData($data,"n_head", ""),"field=n%5Fhead".$keylink,"",MODE_LIST);
			$record["n_head_value"]=$value;

//	n_teaser - 
			$value="";
				$value = ProcessLargeText(GetData($data,"n_teaser", ""),"field=n%5Fteaser".$keylink,"",MODE_LIST);
			$record["n_teaser_value"]=$value;
...
//	n_type - 
			$value="";
				$value = ProcessLargeText(GetData($data,"n_type", ""),"field=n%5Ftype".$keylink,"",MODE_LIST);
			$record["n_type_value"]=$value;

//	n_country - 
			$value="";
				$value=DisplayLookupWizard("n_country",$data["n_country"],$data,$keylink,MODE_LIST);
			$record["n_country_value"]=$value;
...
$xt->assign("logoutlink_attrs","onclick=\"window.location.href='login.php?a=logout';\"");
	...
	$xt->assign("printlink_attrs","href=\"news_print.php\" onclick=\"window.open('news_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"news_print.php?all=1\" onclick=\"window.open('news_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"news_export.php\" onclick=\"window.open('news_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='news_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='news_list.php'; 
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='news_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='news_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	href=\"news_export.php\"");
	
	$xt->assign("add_link",$allow_add);
	$xt->assign("copy_column",$allow_add);
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("addlink_attrs","href=\"news_add.php\" onClick=\"window.location.href='news_add.php'\"");
	$xt->assign("inlineaddlink_attrs","href=\"news_add.php\" onclick=\"return inlineAdd(newrecord_id++);\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"news_edit.php\" disptype=\"control1\" name=\"edit_selected\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
...
$xt->assign("n_teaser_fieldheadercolumn",true);
$xt->assign("n_teaser_fieldcolumn",true);
$xt->assign("n_teaser_fieldfootercolumn",true);
...
$xt->assign("n_country_fieldheadercolumn",true);
$xt->assign("n_country_fieldcolumn",true);
$xt->assign("n_country_fieldfootercolumn",true);
	
$display_grid = $allow_search && $rowsfound;

$xt->assign("asearch_link",$allow_search);
$xt->assign("asearchlink_attrs","href=\"news_search.php\" onclick=\"window.location.href='news_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"news_import.php\" onclick=\"window.location.href='news_import.php';return false;\"");
...
if($display_grid)
{
	...
	"<form method=\"POST\" action=\"news_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
...
$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='news_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
