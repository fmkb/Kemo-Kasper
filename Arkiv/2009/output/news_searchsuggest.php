...
if (isset($_GET['searchFor']) && postvalue('searchFor') != '') {
	...
	$searchField = GoodFieldName( postvalue('searchField') );
			...
			if ( $searchField == '' || $searchField=="n_id")
	{
		$field="n_id";
		...
		$rs=db_query($strSQL,$conn);
			...
			if ( $searchField == '' || $searchField=="n_active")
	{
		$field="n_active";
		if(CheckFieldPermissions($field))
		{
			...
			while ($row = db_fetch_numarray($rs)) {
				...
				if ($pos!==FALSE) {
					...
					$response[] = $row[0];
						...
						if ( $searchField == '' || $searchField=="n_head")
	{
		$field="n_head";
		if(CheckFieldPermissions($field))
		{
		...
		$rs=db_query($strSQL,$conn);
			...
			while ($row = db_fetch_numarray($rs)) {
				...
				if ($pos!==FALSE) {
					...
					if ( $searchField == '' || $searchField=="n_file")
	{
		$field="n_file";
		if(CheckFieldPermissions($field))
		{
		...
		$rs=db_query($strSQL,$conn);
			...
			if ( $searchField == '' || $searchField=="n_type")
	{
		$field="n_type";
		if(CheckFieldPermissions($field))
		{
		...
		$rs=db_query($strSQL,$conn);
			...
			while ($row = db_fetch_numarray($rs)) {
				...
				if ($pos!==FALSE) {
					...
					if ( $searchField == '' || $searchField=="n_country")
	{
		$field="n_country";
		if(CheckFieldPermissions($field))
		{
