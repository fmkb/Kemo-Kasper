...
$xt->assign("langselector_attrs","name=lang onchange=\"javascript: window.location='raffle_list.php?language='+this.options[this.selectedIndex].value\"");
...
"\nvar bLoading=false;\r\n";
	...
	$includes.="var INLINE_EDIT_TABLE='raffle_edit.php';\r\n";
	$includes.="var INLINE_ADD_TABLE='raffle_add.php';\r\n";
	$includes.="var INLINE_VIEW_TABLE='raffle_view.php';\r\n";
	$includes.="var SUGGEST_TABLE='raffle_searchsuggest.php';\r\n";
	$includes.="var MASTER_PREVIEW_TABLE='raffle_masterpreview.php';\r\n";
$includes.="\n</script>\n";
...
$body = array();
$body["begin"]=$includes.'
<form name="frmSearch" method="GET" action="raffle_list.php">
...
$xt->assign("r_id_orderlinkattrs","href=\"raffle_list.php?orderby=ar_id\"");
$xt->assign("r_id_fieldheader",true);
$xt->assign("r_name_orderlinkattrs","href=\"raffle_list.php?orderby=ar_name\"");
$xt->assign("r_name_fieldheader",true);
$xt->assign("r_img_orderlinkattrs","href=\"raffle_list.php?orderby=ar_img\"");
$xt->assign("r_img_fieldheader",true);
$xt->assign("r_date_orderlinkattrs","href=\"raffle_list.php?orderby=ar_date\"");
$xt->assign("r_date_fieldheader",true);
$xt->assign("r_p_id_orderlinkattrs","href=\"raffle_list.php?orderby=ar_p_id\"");
...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	$xt->assign(GoodFieldName($order_field)."_orderlinkattrs","href=\"raffle_list.php?orderby=".$dir.GoodFieldName($order_field)."\"");
...
{  
	...
	{
		$strWhere = "1=0";
		if($where=StrWhere("r_id", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("r_name", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("r_img", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("r_text", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("r_date", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("r_p_id", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
...
//	write pagination
	if($maxpages>1)
	{
		...
		{
			window.location='raffle_list.php?goto='+nPageNumber;
		}
		</script>");
	}
...
//	key fields
	...
	$copylink="";
	$keylink="";
	$keyblock.= rawurlencode($data["r_id"]);
	$editlink.="editid1=".htmlspecialchars(rawurlencode($data["r_id"]));
	$copylink.="copyid1=".htmlspecialchars(rawurlencode($data["r_id"]));
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["r_id"]));

	$record["editlink_attrs"]="href=\"raffle_edit.php?".$editlink."\" id=\"editlink".$recno."\"";
	$record["inlineeditlink_attrs"]= "href=\"raffle_edit.php?".$editlink."\" onclick=\"return inlineEdit('".$recno."','".$editlink."');\" id=\"ieditlink".$recno."\"";
	$record["copylink_attrs"]="href=\"raffle_add.php?".$copylink."\" id=\"copylink".$recno."\"";
	$record["viewlink_attrs"]="href=\"raffle_view.php?".$editlink."\" id=\"viewlink".$recno."\"";
	$record["checkbox_attrs"]="name=\"selection[]\" value=\"".$keyblock."\" id=\"check".$recno."\"";


//	r_id - 
			$value="";
				$value = ProcessLargeText(GetData($data,"r_id", ""),"field=r%5Fid".$keylink,"",MODE_LIST);
			$record["r_id_value"]=$value;

//	r_name - 
			$value="";
				$value = ProcessLargeText(GetData($data,"r_name", ""),"field=r%5Fname".$keylink,"",MODE_LIST);
			$record["r_name_value"]=$value;

//	r_img - 
			$value="";
				$value = ProcessLargeText(GetData($data,"r_img", ""),"field=r%5Fimg".$keylink,"",MODE_LIST);
			$record["r_img_value"]=$value;

//	r_date - Short Date
			$value="";
				$value = ProcessLargeText(GetData($data,"r_date", "Short Date"),"field=r%5Fdate".$keylink,"",MODE_LIST);
			$record["r_date_value"]=$value;

//	r_p_id - 
			$value="";
				$value=DisplayLookupWizard("r_p_id",$data["r_p_id"],$data,$keylink,MODE_LIST);
...
$xt->assign("logoutlink_attrs","onclick=\"window.location.href='login.php?a=logout';\"");
	...
	$xt->assign("printlink_attrs","href=\"raffle_print.php\" onclick=\"window.open('raffle_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"raffle_print.php?all=1\" onclick=\"window.open('raffle_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"raffle_export.php\" onclick=\"window.open('raffle_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='raffle_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='raffle_list.php'; 
	document.forms.frmAdmin.target='_self';return false\"
	href=\"raffle_print.php\"");
	$xt->assign("exportselected_link",$allow_export);
	$xt->assign("exportselectedlink_attrs","disptype=\"control1\" onclick=\"
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='raffle_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='raffle_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	href=\"raffle_export.php\"");
	
	$xt->assign("add_link",$allow_add);
	$xt->assign("copy_column",$allow_add);
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("addlink_attrs","href=\"raffle_add.php\" onClick=\"window.location.href='raffle_add.php'\"");
	$xt->assign("inlineaddlink_attrs","href=\"raffle_add.php\" onclick=\"return inlineAdd(newrecord_id++);\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"raffle_edit.php\" disptype=\"control1\" name=\"edit_selected\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
...
$xt->assign("asearchlink_attrs","href=\"raffle_search.php\" onclick=\"window.location.href='raffle_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"raffle_import.php\" onclick=\"window.location.href='raffle_import.php';return false;\"");
...
if($display_grid)
{
	...
	"<form method=\"POST\" action=\"raffle_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
...
$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='raffle_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
