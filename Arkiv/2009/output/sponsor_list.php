...
include("include/sponsor_variables.php");
	include("include/teams_settings.php");
...
$xt->assign("langselector_attrs","name=lang onchange=\"javascript: window.location='sponsor_list.php?language='+this.options[this.selectedIndex].value\"");
...
"\nvar bLoading=false;\r\n";
	...
	$includes.="var INLINE_EDIT_TABLE='sponsor_edit.php';\r\n";
	$includes.="var INLINE_ADD_TABLE='sponsor_add.php';\r\n";
	$includes.="var INLINE_VIEW_TABLE='sponsor_view.php';\r\n";
	$includes.="var SUGGEST_TABLE='sponsor_searchsuggest.php';\r\n";
	$includes.="var MASTER_PREVIEW_TABLE='sponsor_masterpreview.php';\r\n";
$includes.="\n</script>\n";
...
$body = array();
$body["begin"]=$includes.'
<form name="frmSearch" method="GET" action="sponsor_list.php">
...
$xt->assign("s_active_orderlinkattrs","href=\"sponsor_list.php?orderby=as_active\"");
$xt->assign("s_active_fieldheader",true);
$xt->assign("s_name_orderlinkattrs","href=\"sponsor_list.php?orderby=as_name\"");
$xt->assign("s_name_fieldheader",true);
$xt->assign("s_contact_orderlinkattrs","href=\"sponsor_list.php?orderby=as_contact\"");
$xt->assign("s_contact_fieldheader",true);
$xt->assign("s_phone1_orderlinkattrs","href=\"sponsor_list.php?orderby=as_phone1\"");
$xt->assign("s_phone1_fieldheader",true);
$xt->assign("s_banner_orderlinkattrs","href=\"sponsor_list.php?orderby=as_banner\"");
$xt->assign("s_banner_fieldheader",true);
$xt->assign("s_www_orderlinkattrs","href=\"sponsor_list.php?orderby=as_www\"");
$xt->assign("s_www_fieldheader",true);
$xt->assign("s_mail_orderlinkattrs","href=\"sponsor_list.php?orderby=as_mail\"");
$xt->assign("s_mail_fieldheader",true);
$xt->assign("s_country_orderlinkattrs","href=\"sponsor_list.php?orderby=as_country\"");
...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	$xt->assign(GoodFieldName($order_field)."_orderlinkattrs","href=\"sponsor_list.php?orderby=".$dir.GoodFieldName($order_field)."\"");
...
{  
	...
	{
		$strWhere = "1=0";
		if($where=StrWhere("s_active", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("s_name", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("s_contact", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("s_adr", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("s_zip", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("s_phone1", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("s_phone2", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("s_total", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("s_paid", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("s_www", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("s_mail", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("s_cmt", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("s_country", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
...
if($allow_search)
{
	...
	$countsql = "select count(*) from ".AddTableWrappers($strOriginalDetailsTable)." where ".$sqlWhere;
		$gsqlHead.=",(".$countsql.") as teams_cnt ";
	}
	$strSQLbak = $strSQL;
...
//	write pagination
	if($maxpages>1)
	{
		...
		{
			window.location='sponsor_list.php?goto='+nPageNumber;
		}
		</script>");
	}
...
//	detail tables
			$masterquery="mastertable=sponsor";
	
			$detailid=array();
			$masterquery.="&masterkey1=".rawurlencode($data["s_id"]);
			$detailid[]=make_db_value("s_id",$data["s_id"],"","","teams");
	//	add count of child records to SQL
		if(!$bSubqueriesSupported)
	{
	...
	foreach($masterkeys as $idx=>$val)
	{
		if($masterwhere)
			$masterwhere.=" and ";
	$masterwhere.=GetFullFieldName($detailkeys[$idx],"teams")."=".$detailid[$idx];
	}
	$data["teams_cnt"]=gSQLRowCount_int($sqlHead,$sqlFrom,$sqlWhere,$sqlTail,$masterwhere,0);
	}

//detail tables
	$record["teams_dtable_link"]=$allow_teams;
	$record["teams_dtablelink_attrs"]="href=\"teams_list.php?".$masterquery."\" id=\"master_teams".$recno."\"";
	$dpreview=true;
		if($data["teams_cnt"]+0)
		$record["teams_childcount"]=true;
	else
		$dpreview=false;
	$record["teams_childnumber"]=$data["teams_cnt"];
		if($dpreview)
	{
			$record["teams_dtablelink_attrs"].=" onmouseover=\"RollDetailsLink.showPopup(this,'teams_detailspreview.php'+this.href.substr(this.href.indexOf('?')));\" onmouseout=\"RollDetailsLink.hidePopup();\"";
		}
...
//	key fields
	...
	$copylink="";
	$keylink="";
	$keyblock.= rawurlencode($data["s_id"]);
	$editlink.="editid1=".htmlspecialchars(rawurlencode($data["s_id"]));
	$copylink.="copyid1=".htmlspecialchars(rawurlencode($data["s_id"]));
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["s_id"]));

	$record["editlink_attrs"]="href=\"sponsor_edit.php?".$editlink."\" id=\"editlink".$recno."\"";
	$record["inlineeditlink_attrs"]= "href=\"sponsor_edit.php?".$editlink."\" onclick=\"return inlineEdit('".$recno."','".$editlink."');\" id=\"ieditlink".$recno."\"";
	$record["copylink_attrs"]="href=\"sponsor_add.php?".$copylink."\" id=\"copylink".$recno."\"";
	$record["viewlink_attrs"]="href=\"sponsor_view.php?".$editlink."\" id=\"viewlink".$recno."\"";
...
//	s_name - 
			$value="";
				$value = ProcessLargeText(GetData($data,"s_name", ""),"field=s%5Fname".$keylink,"",MODE_LIST);
			$record["s_name_value"]=$value;

//	s_contact - 
			$value="";
				$value = ProcessLargeText(GetData($data,"s_contact", ""),"field=s%5Fcontact".$keylink,"",MODE_LIST);
...
//	s_phone1 - 
			$value="";
				$value = ProcessLargeText(GetData($data,"s_phone1", ""),"field=s%5Fphone1".$keylink,"",MODE_LIST);
...
$xt->assign("logoutlink_attrs","onclick=\"window.location.href='login.php?a=logout';\"");
	...
	$xt->assign("printlink_attrs","href=\"sponsor_print.php\" onclick=\"window.open('sponsor_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"sponsor_print.php?all=1\" onclick=\"window.open('sponsor_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"sponsor_export.php\" onclick=\"window.open('sponsor_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='sponsor_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='sponsor_list.php'; 
	document.forms.frmAdmin.target='_self';return false\"
	href=\"sponsor_print.php\"");
	$xt->assign("exportselected_link",$allow_export);
	$xt->assign("exportselectedlink_attrs","disptype=\"control1\" onclick=\"
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='sponsor_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='sponsor_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	href=\"sponsor_export.php\"");
	
	$xt->assign("add_link",$allow_add);
	$xt->assign("copy_column",$allow_add);
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("addlink_attrs","href=\"sponsor_add.php\" onClick=\"window.location.href='sponsor_add.php'\"");
	$xt->assign("inlineaddlink_attrs","href=\"sponsor_add.php\" onclick=\"return inlineAdd(newrecord_id++);\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"sponsor_edit.php\" disptype=\"control1\" name=\"edit_selected\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
	...
	$xt->assign("view_column",$allow_search);
 	...
 	$xt->assign("teams_dtable_column",$allow_teams);
...
$xt->assign("asearchlink_attrs","href=\"sponsor_search.php\" onclick=\"window.location.href='sponsor_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"sponsor_import.php\" onclick=\"window.location.href='sponsor_import.php';return false;\"");
...
if($display_grid)
{
	...
	"<form method=\"POST\" action=\"sponsor_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
...
$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='sponsor_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
