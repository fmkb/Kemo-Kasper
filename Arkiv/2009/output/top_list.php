...
$xt->assign("langselector_attrs","name=lang onchange=\"javascript: window.location='top_list.php?language='+this.options[this.selectedIndex].value\"");
...
"\nvar bLoading=false;\r\n";
	...
	$includes.="var INLINE_EDIT_TABLE='top_edit.php';\r\n";
	$includes.="var INLINE_ADD_TABLE='top_add.php';\r\n";
	$includes.="var INLINE_VIEW_TABLE='top_view.php';\r\n";
	$includes.="var SUGGEST_TABLE='top_searchsuggest.php';\r\n";
	$includes.="var MASTER_PREVIEW_TABLE='top_masterpreview.php';\r\n";
$includes.="\n</script>\n";
...
$body = array();
$body["begin"]=$includes.'
<form name="frmSearch" method="GET" action="top_list.php">
...
$xt->assign("t_id_orderlinkattrs","href=\"top_list.php?orderby=at_id\"");
$xt->assign("t_id_fieldheader",true);
$xt->assign("t_user_orderlinkattrs","href=\"top_list.php?orderby=at_user\"");
$xt->assign("t_user_fieldheader",true);
$xt->assign("t_score_orderlinkattrs","href=\"top_list.php?orderby=at_score\"");
$xt->assign("t_score_fieldheader",true);
$xt->assign("t_datetime_orderlinkattrs","href=\"top_list.php?orderby=at_datetime\"");
$xt->assign("t_datetime_fieldheader",true);
$xt->assign("t_ip_orderlinkattrs","href=\"top_list.php?orderby=at_ip\"");
$xt->assign("t_ip_fieldheader",true);
$xt->assign("t_p_id_orderlinkattrs","href=\"top_list.php?orderby=at_p_id\"");
$xt->assign("t_p_id_fieldheader",true);
$xt->assign("t_ts_id_orderlinkattrs","href=\"top_list.php?orderby=at_ts_id\"");
$xt->assign("t_ts_id_fieldheader",true);
$xt->assign("t_kills_orderlinkattrs","href=\"top_list.php?orderby=at_kills\"");
...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	$xt->assign(GoodFieldName($order_field)."_orderlinkattrs","href=\"top_list.php?orderby=".$dir.GoodFieldName($order_field)."\"");
...
{  
	...
	{
		$strWhere = "1=0";
		if($where=StrWhere("t_id", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("t_user", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("t_score", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("t_datetime", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("t_ip", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("t_p_id", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("t_ts_id", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("t_kills", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
...
//	write pagination
	if($maxpages>1)
	{
		...
		{
			window.location='top_list.php?goto='+nPageNumber;
		}
		</script>");
	}
...
//	key fields
	...
	$copylink="";
	$keylink="";
	$keyblock.= rawurlencode($data["t_id"]);
	$editlink.="editid1=".htmlspecialchars(rawurlencode($data["t_id"]));
	$copylink.="copyid1=".htmlspecialchars(rawurlencode($data["t_id"]));
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["t_id"]));

	$record["editlink_attrs"]="href=\"top_edit.php?".$editlink."\" id=\"editlink".$recno."\"";
	$record["inlineeditlink_attrs"]= "href=\"top_edit.php?".$editlink."\" onclick=\"return inlineEdit('".$recno."','".$editlink."');\" id=\"ieditlink".$recno."\"";
	$record["copylink_attrs"]="href=\"top_add.php?".$copylink."\" id=\"copylink".$recno."\"";
	$record["viewlink_attrs"]="href=\"top_view.php?".$editlink."\" id=\"viewlink".$recno."\"";
	$record["checkbox_attrs"]="name=\"selection[]\" value=\"".$keyblock."\" id=\"check".$recno."\"";


//	t_id - 
			$value="";
				$value = ProcessLargeText(GetData($data,"t_id", ""),"field=t%5Fid".$keylink,"",MODE_LIST);
			$record["t_id_value"]=$value;

//	t_datetime - Short Date
			$value="";
				$value = ProcessLargeText(GetData($data,"t_datetime", "Short Date"),"field=t%5Fdatetime".$keylink,"",MODE_LIST);
			$record["t_datetime_value"]=$value;

//	t_ts_id - 
			$value="";
				$value = ProcessLargeText(GetData($data,"t_ts_id", ""),"field=t%5Fts%5Fid".$keylink,"",MODE_LIST);
			$record["t_ts_id_value"]=$value;

//	t_p_id - 
			$value="";
				$value=DisplayLookupWizard("t_p_id",$data["t_p_id"],$data,$keylink,MODE_LIST);
			$record["t_p_id_value"]=$value;

//	t_score - 
			$value="";
				$value = ProcessLargeText(GetData($data,"t_score", ""),"field=t%5Fscore".$keylink,"",MODE_LIST);
			$record["t_score_value"]=$value;

//	t_kills - 
			$value="";
				$value = ProcessLargeText(GetData($data,"t_kills", ""),"field=t%5Fkills".$keylink,"",MODE_LIST);
			$record["t_kills_value"]=$value;

//	t_user - 
			$value="";
				$value = ProcessLargeText(GetData($data,"t_user", ""),"field=t%5Fuser".$keylink,"",MODE_LIST);
			$record["t_user_value"]=$value;

//	t_ip - 
			$value="";
				$value = ProcessLargeText(GetData($data,"t_ip", ""),"field=t%5Fip".$keylink,"",MODE_LIST);
...
$xt->assign("logoutlink_attrs","onclick=\"window.location.href='login.php?a=logout';\"");
	...
	$xt->assign("printlink_attrs","href=\"top_print.php\" onclick=\"window.open('top_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"top_print.php?all=1\" onclick=\"window.open('top_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"top_export.php\" onclick=\"window.open('top_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='top_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='top_list.php'; 
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='top_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='top_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	...
	$xt->assign("addlink_attrs","href=\"top_add.php\" onClick=\"window.location.href='top_add.php'\"");
	$xt->assign("inlineaddlink_attrs","href=\"top_add.php\" onclick=\"return inlineAdd(newrecord_id++);\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"top_edit.php\" disptype=\"control1\" name=\"edit_selected\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
...
$xt->assign("asearchlink_attrs","href=\"top_search.php\" onclick=\"window.location.href='top_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"top_import.php\" onclick=\"window.location.href='top_import.php';return false;\"");
...
if($display_grid)
{
	...
	"<form method=\"POST\" action=\"top_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
...
$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='top_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
...
//	include proper masterlist.php code
	include("include/player_masterlist.php");
	...
	$xt->assign("backtomasterlink_attrs","href=\"player_list.php?a=return\"");
