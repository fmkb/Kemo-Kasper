...
$xt->assign("langselector_attrs","name=lang onchange=\"javascript: window.location='xcountries_list.php?language='+this.options[this.selectedIndex].value\"");
...
"\nvar bLoading=false;\r\n";
	...
	$includes.="var INLINE_EDIT_TABLE='xcountries_edit.php';\r\n";
	$includes.="var INLINE_ADD_TABLE='xcountries_add.php';\r\n";
	$includes.="var INLINE_VIEW_TABLE='xcountries_view.php';\r\n";
	$includes.="var SUGGEST_TABLE='xcountries_searchsuggest.php';\r\n";
	$includes.="var MASTER_PREVIEW_TABLE='xcountries_masterpreview.php';\r\n";
$includes.="\n</script>\n";
...
$body = array();
$body["begin"]=$includes.'
<form name="frmSearch" method="GET" action="xcountries_list.php">
...
$xt->assign("xc_id_orderlinkattrs","href=\"xcountries_list.php?orderby=axc_id\"");
$xt->assign("xc_id_fieldheader",true);
$xt->assign("xc_name_orderlinkattrs","href=\"xcountries_list.php?orderby=axc_name\"");
$xt->assign("xc_name_fieldheader",true);
$xt->assign("xc_currency_orderlinkattrs","href=\"xcountries_list.php?orderby=axc_currency\"");
$xt->assign("xc_currency_fieldheader",true);
$xt->assign("xc_code_orderlinkattrs","href=\"xcountries_list.php?orderby=axc_code\"");
$xt->assign("xc_code_fieldheader",true);
...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	$xt->assign(GoodFieldName($order_field)."_orderlinkattrs","href=\"xcountries_list.php?orderby=".$dir.GoodFieldName($order_field)."\"");
...
if (@$_REQUEST["mdelete"])
{
	...
	{
		$keys=array();
		$keys["xc_id"]=refine($_REQUEST["mdelete1"][$ind-1]);
		$selected_recs[]=$keys;
	}
}
...
{  
	...
	{
		$strWhere = "1=0";
		if($where=StrWhere("xc_id", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("xc_name", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("xc_currency", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("xc_code", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("xc_desc", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
...
//	write pagination
	if($maxpages>1)
	{
		...
		{
			window.location='xcountries_list.php?goto='+nPageNumber;
		}
		</script>");
	}
...
//	key fields
	...
	$copylink="";
	$keylink="";
	$keyblock.= rawurlencode($data["xc_id"]);
	$editlink.="editid1=".htmlspecialchars(rawurlencode($data["xc_id"]));
	$copylink.="copyid1=".htmlspecialchars(rawurlencode($data["xc_id"]));
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["xc_id"]));

	$record["editlink_attrs"]="href=\"xcountries_edit.php?".$editlink."\" id=\"editlink".$recno."\"";
	$record["inlineeditlink_attrs"]= "href=\"xcountries_edit.php?".$editlink."\" onclick=\"return inlineEdit('".$recno."','".$editlink."');\" id=\"ieditlink".$recno."\"";
	$record["copylink_attrs"]="href=\"xcountries_add.php?".$copylink."\" id=\"copylink".$recno."\"";
	$record["viewlink_attrs"]="href=\"xcountries_view.php?".$editlink."\" id=\"viewlink".$recno."\"";
	$record["checkbox_attrs"]="name=\"selection[]\" value=\"".$keyblock."\" id=\"check".$recno."\"";


//	xc_id - 
			$value="";
				$value = ProcessLargeText(GetData($data,"xc_id", ""),"field=xc%5Fid".$keylink,"",MODE_LIST);
			$record["xc_id_value"]=$value;

//	xc_name - 
			$value="";
				$value = ProcessLargeText(GetData($data,"xc_name", ""),"field=xc%5Fname".$keylink,"",MODE_LIST);
			$record["xc_name_value"]=$value;

//	xc_currency - 
			$value="";
				$value = ProcessLargeText(GetData($data,"xc_currency", ""),"field=xc%5Fcurrency".$keylink,"",MODE_LIST);
			$record["xc_currency_value"]=$value;

//	xc_code - 
			$value="";
				$value = ProcessLargeText(GetData($data,"xc_code", ""),"field=xc%5Fcode".$keylink,"",MODE_LIST);
			$record["xc_code_value"]=$value;
...
$xt->assign("logoutlink_attrs","onclick=\"window.location.href='login.php?a=logout';\"");
	...
	$xt->assign("printlink_attrs","href=\"xcountries_print.php\" onclick=\"window.open('xcountries_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"xcountries_print.php?all=1\" onclick=\"window.open('xcountries_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"xcountries_export.php\" onclick=\"window.open('xcountries_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='xcountries_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='xcountries_list.php'; 
	document.forms.frmAdmin.target='_self';return false\"
	href=\"xcountries_print.php\"");
	$xt->assign("exportselected_link",$allow_export);
	$xt->assign("exportselectedlink_attrs","disptype=\"control1\" onclick=\"
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='xcountries_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='xcountries_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	href=\"xcountries_export.php\"");
	
	$xt->assign("add_link",$allow_add);
	$xt->assign("copy_column",$allow_add);
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("addlink_attrs","href=\"xcountries_add.php\" onClick=\"window.location.href='xcountries_add.php'\"");
	$xt->assign("inlineaddlink_attrs","href=\"xcountries_add.php\" onclick=\"return inlineAdd(newrecord_id++);\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"xcountries_edit.php\" disptype=\"control1\" name=\"edit_selected\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
...
$xt->assign("xc_id_fieldheadercolumn",true);
$xt->assign("xc_id_fieldcolumn",true);
$xt->assign("xc_id_fieldfootercolumn",true);
$xt->assign("xc_name_fieldheadercolumn",true);
$xt->assign("xc_name_fieldcolumn",true);
$xt->assign("xc_name_fieldfootercolumn",true);
$xt->assign("xc_currency_fieldheadercolumn",true);
$xt->assign("xc_currency_fieldcolumn",true);
$xt->assign("xc_currency_fieldfootercolumn",true);
$xt->assign("xc_code_fieldheadercolumn",true);
$xt->assign("xc_code_fieldcolumn",true);
$xt->assign("xc_code_fieldfootercolumn",true);
	
$display_grid = $allow_search && $rowsfound;

$xt->assign("asearch_link",$allow_search);
$xt->assign("asearchlink_attrs","href=\"xcountries_search.php\" onclick=\"window.location.href='xcountries_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"xcountries_import.php\" onclick=\"window.location.href='xcountries_import.php';return false;\"");
...
if($display_grid)
{
	...
	"<form method=\"POST\" action=\"xcountries_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
...
$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='xcountries_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
...
$templatefile = "xcountries_list.htm";
