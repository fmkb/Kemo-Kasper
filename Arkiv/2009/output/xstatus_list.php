...
$xt->assign("langselector_attrs","name=lang onchange=\"javascript: window.location='xstatus_list.php?language='+this.options[this.selectedIndex].value\"");
...
//	validation stuff
	...
	$includes.="<script type=\"text/javascript\" src=\"include/inlineedit.js\"></script>\r\n";
			$editValidateTypes[] = "";
		$editValidateFields[] = "xs_text";
	
			$addValidateTypes[] = "";
		$addValidateFields[] = "xs_text";
...
"\nvar bLoading=false;\r\n";
	...
	$includes.="var INLINE_EDIT_TABLE='xstatus_edit.php';\r\n";
	$includes.="var INLINE_ADD_TABLE='xstatus_add.php';\r\n";
	$includes.="var INLINE_VIEW_TABLE='xstatus_view.php';\r\n";
	$includes.="var SUGGEST_TABLE='xstatus_searchsuggest.php';\r\n";
	$includes.="var MASTER_PREVIEW_TABLE='xstatus_masterpreview.php';\r\n";
$includes.="\n</script>\n";
...
$body = array();
$body["begin"]=$includes.'
<form name="frmSearch" method="GET" action="xstatus_list.php">
...
$xt->assign("xs_id_orderlinkattrs","href=\"xstatus_list.php?orderby=axs_id\"");
$xt->assign("xs_id_fieldheader",true);
$xt->assign("xs_text_orderlinkattrs","href=\"xstatus_list.php?orderby=axs_text\"");
$xt->assign("xs_text_fieldheader",true);
...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	$xt->assign(GoodFieldName($order_field)."_orderlinkattrs","href=\"xstatus_list.php?orderby=".$dir.GoodFieldName($order_field)."\"");
...
if (@$_REQUEST["mdelete"])
{
	...
	{
		$keys=array();
		$keys["xs_id"]=refine($_REQUEST["mdelete1"][$ind-1]);
		$selected_recs[]=$keys;
	}
}
...
{
	$createmenu=true;
	$xt->assign("xstatus_tablelink",true);
	$page="";
		$page="list";
		$xt->assign("xstatus_tablelink_attrs","href=\"xstatus_".$page.".php\"");
	$xt->assign("xstatus_optionattrs","value=\"xstatus_".$page.".php\"");
}
if($allow_ZZIP)
{
	$createmenu=true;
	$xt->assign("ZZIP_tablelink",true);
	$page="";
		$page="list";
		$xt->assign("ZZIP_tablelink_attrs","href=\"ZZIP_".$page.".php\"");
	$xt->assign("ZZIP_optionattrs","value=\"ZZIP_".$page.".php\"");
}
if($allow_Q1)
{
	$createmenu=true;
	$xt->assign("Q1_tablelink",true);
	$page="";
		$page="list";
		$strPerm = GetUserPermissions("Q1");
	if(strpos($strPerm, "A")!==false && strpos($strPerm, "S")===false)
		$page="add";
	$xt->assign("Q1_tablelink_attrs","href=\"Q1_".$page.".php\"");
	$xt->assign("Q1_optionattrs","value=\"Q1_".$page.".php\"");
}
if($createmenu)
...
{  
	...
	{
		$strWhere = "1=0";
		if($where=StrWhere("xs_id", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("xs_text", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
...
//	write pagination
	if($maxpages>1)
	{
		...
		{
			window.location='xstatus_list.php?goto='+nPageNumber;
		}
		</script>");
	}
...
//	add inline add row	
		...
		$record["xs_id_value"] = "<span id=\"add_xs_id\">&nbsp;</span>";
		$record["xs_text_value"] = "<span id=\"add_xs_text\">&nbsp;</span>";
		if($colsonpage>1)
...
//	key fields
	...
	$copylink="";
	$keylink="";
	$keyblock.= rawurlencode($data["xs_id"]);
	$editlink.="editid1=".htmlspecialchars(rawurlencode($data["xs_id"]));
	$copylink.="copyid1=".htmlspecialchars(rawurlencode($data["xs_id"]));
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["xs_id"]));

	$record["editlink_attrs"]="href=\"xstatus_edit.php?".$editlink."\" id=\"editlink".$recno."\"";
	$record["inlineeditlink_attrs"]= "href=\"xstatus_edit.php?".$editlink."\" onclick=\"return inlineEdit('".$recno."','".$editlink."');\" id=\"ieditlink".$recno."\"";
	$record["copylink_attrs"]="href=\"xstatus_add.php?".$copylink."\" id=\"copylink".$recno."\"";
	$record["viewlink_attrs"]="href=\"xstatus_view.php?".$editlink."\" id=\"viewlink".$recno."\"";
	$record["checkbox_attrs"]="name=\"selection[]\" value=\"".$keyblock."\" id=\"check".$recno."\"";


//	xs_id - 
			$value="";
				$value = ProcessLargeText(GetData($data,"xs_id", ""),"field=xs%5Fid".$keylink,"",MODE_LIST);
			$record["xs_id_value"]=$value;

//	xs_text - 
			$value="";
				$value = ProcessLargeText(GetData($data,"xs_text", ""),"field=xs%5Ftext".$keylink,"",MODE_LIST);
			$record["xs_text_value"]=$value;
			if(function_exists("BeforeMoveNextList"))
				BeforeMoveNextList($data,$record,$col);
			$span="<span ";
			$span.="id=\"edit".$recno."_xs_id\" ";
					$span.=">";
			$record["xs_id_value"] = $span.$record["xs_id_value"]."</span>";
			$span="<span ";
			$span.="id=\"edit".$recno."_xs_text\" ";
					$span.=">";
			$record["xs_text_value"] = $span.$record["xs_text_value"]."</span>";
			if($col<$colsonpage)
				$record["endrecord_block"]=true;
			$record["grid_recordheader"]=true;
			$record["grid_vrecord"]=true;
...
$xt->assign("logoutlink_attrs","onclick=\"window.location.href='login.php?a=logout';\"");
	...
	$xt->assign("printlink_attrs","href=\"xstatus_print.php\" onclick=\"window.open('xstatus_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"xstatus_print.php?all=1\" onclick=\"window.open('xstatus_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"xstatus_export.php\" onclick=\"window.open('xstatus_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='xstatus_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='xstatus_list.php'; 
	document.forms.frmAdmin.target='_self';return false\"
	href=\"xstatus_print.php\"");
	$xt->assign("exportselected_link",$allow_export);
	$xt->assign("exportselectedlink_attrs","disptype=\"control1\" onclick=\"
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='xstatus_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='xstatus_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	href=\"xstatus_export.php\"");
	
	$xt->assign("add_link",$allow_add);
	$xt->assign("copy_column",$allow_add);
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("addlink_attrs","href=\"xstatus_add.php\" onClick=\"window.location.href='xstatus_add.php'\"");
	$xt->assign("inlineaddlink_attrs","href=\"xstatus_add.php\" onclick=\"return inlineAdd(newrecord_id++);\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"xstatus_edit.php\" disptype=\"control1\" name=\"edit_selected\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
...
$xt->assign("xs_id_fieldheadercolumn",true);
$xt->assign("xs_id_fieldcolumn",true);
$xt->assign("xs_id_fieldfootercolumn",true);
$xt->assign("xs_text_fieldheadercolumn",true);
$xt->assign("xs_text_fieldcolumn",true);
$xt->assign("xs_text_fieldfootercolumn",true);
	
$display_grid = $allow_add || $allow_search && $rowsfound;

$xt->assign("asearch_link",$allow_search);
$xt->assign("asearchlink_attrs","href=\"xstatus_search.php\" onclick=\"window.location.href='xstatus_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"xstatus_import.php\" onclick=\"window.location.href='xstatus_import.php';return false;\"");
...
if($display_grid)
{
	...
	"<form method=\"POST\" action=\"xstatus_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
...
$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='xstatus_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
